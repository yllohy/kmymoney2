...
MyMoneyStorageGNC::MyMoneyStorageGNC()
{
  m_storage = NULL;
  m_doc     = NULL;
  m_mainAssetId = QString("AStd::Asset");
  m_mainLiabilityId = QString("AStd::Liability");
  m_mainIncomeId = QString("AStd::Income");
  m_mainExpenseId = QString("AStd::Expense");
  m_mainEquityId = QString("AStd::Asset");
}
      ...
      */
      ...
      for(theAccount = list.begin(); theAccount != list.end(); ++theAccount)
      {
        if((*theAccount).parentAccountId() == QCString(m_mainAssetId))
        {
          ...
          if (gncdebug) qDebug("The account id %s is a child of the main asset account", (*theAccount).id().data());
        }
        else if((*theAccount).parentAccountId() == QCString(m_mainLiabilityId))
        {
          ...
          if (gncdebug) qDebug("The account id %s is a child of the main liability account", (*theAccount).id().data());      }
        ...
        if (gncdebug) qDebug("The account id %s is a child of the main income account", (*theAccount).id().data());
        }
        else if((*theAccount).parentAccountId() == QCString(m_mainExpenseId))
        {
          ...
          if (gncdebug) qDebug("The account id %s is a child of the main expense account", (*theAccount).id().data());
        }
        ...
        else
        {
          ...
          if(id != m_mapIds.end())
          {
            if (gncdebug) qDebug("Setting account id %s's parent account id to %s", (*theAccount).id().data(), id.data().data());
      ...
      // send a warning re scheuled txs
      if (containsScheds)
...
void MyMoneyStorageGNC::readAccount(const QDomElement& account)
{
  ...
  if(QString("2.0.0") == gncVersion)
  {
    ...
    if(QString("BANK") == gncType)
    {
      ...
      if (!bHasParent)
        acc.setParentAccountId(QCString(m_mainAssetId));
    }
    ...
    else if(QString("LIABILITY") == gncType)
    {
      ...
      if (!bHasParent)
        acc.setParentAccountId(QCString(m_mainLiabilityId));
    }
    ...
    else if(QString("INCOME") == gncType)
    {
      ...
      if (!bHasParent)
        acc.setParentAccountId(QCString(m_mainIncomeId));
    }
    else if(QString("EXPENSE") == gncType)
    {
      acc.setAccountType(MyMoneyAccount::Expense);
      if (!bHasParent)
        acc.setParentAccountId(QCString(m_mainExpenseId));
    }
...
void MyMoneyStorageGNC::readSplits(MyMoneyTransaction& tx, QDomElement& splits)
{
  ...
  // first off, is it a ntransfer (can only have 2 splits?)
...
//******************************** readSplit ***************************
void MyMoneyStorageGNC::saveSplits (MyMoneySplit s)
{
...
MyMoneySplit MyMoneyStorageGNC::readSplit(MyMoneyTransaction& tx, QDomElement& splitElement)
{
  ...
  // action
  //if the money amount is negative, mark it as a withdrawal, otherwise it's a deposit.
  ...
  m_splitAccount = *it;  // save it for later
