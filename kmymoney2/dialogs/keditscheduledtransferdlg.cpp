...
KEditScheduledTransferDlg::KEditScheduledTransferDlg(MyMoneyFile *file, QWidget *parent, const char *name)
...
/*
  ...
  m_kcomboTo->insertStringList(qstringlistAccounts);
  m_kcomboFrom->insertStringList(qstringlistAccounts);
...
/*
  ...
  QString qstringBankName =
    m_kcomboTo->currentText().mid(1, m_kcomboTo->currentText().find(':')-1);
  ...
  for (bank=m_mymoneyfile->bankFirst(); bank; bank=m_mymoneyfile->bankNext())
  {
    if (bank->name() == qstringBankName)
    {
      QString qstringAccountName =
        m_kcomboTo->currentText().mid(
          m_kcomboTo->currentText().find(':')+1,
          m_kcomboTo->currentText().length() - (qstringBankName.length()+3) );
